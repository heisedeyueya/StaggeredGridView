 ...
 * 
 * <p>
 * StaggeredGridView presents a multi-column grid with consistent column sizes but varying row sizes between the
 * columns. Each successive item from a {@link android.widget.ListAdapter ListAdapter} will be arranged from top to
 * bottom, left to right. The largest vertical gap is always filled first.
 * </p>
 * 
 * <p>
 * Item views may span multiple columns as specified by their {@link LayoutParams}. The attribute
 * <code>android:layout_span</code> may be used when inflating item views from xml.
 * </p>
 */
public class StaggeredGridView extends ViewGroup {
     ...
     * 
     * Like ListView, SGV populates from an adapter and recycles views that fall out of the visible boundaries of the
     * grid. A few invariants always hold:
     * 
     * - mFirstPosition is the adapter position of the View returned by getChildAt(0). - Any child index can be
     * translated to an adapter position by adding mFirstPosition. - Any adapter position can be translated to a child
     * index by subtracting mFirstPosition. - Views for items in the range [mFirstPosition, mFirstPosition +
     * getChildCount()) are currently attached to the grid as children. All other adapter positions do not have active
     * views.
     * 
     * This means a few things thanks to the staggered grid's nature. Some views may stay attached long after they have
     * scrolled offscreen if removing and recycling them would result in breaking one of the invariants above.
     * 
     * LayoutRecords are used to track data about a particular item's layout after the associated view has been removed.
     * These let positioning and the choice of column for an item remain consistent even though the rules for filling
     * content up vs. filling down vary.
     * 
     * Whenever layout parameters for a known LayoutRecord change, other LayoutRecords before or after it may need to be
     * invalidated. e.g. if the item's height or the number of columns it spans changes, all bets for other items in the
     * same direction are off since the cached information no longer applies.
     */
    ...
    /**
     * Set a fixed number of columns for this grid. Space will be divided evenly among all columns, respecting the item
     * margin between columns. The default is 2. (If it were 1, perhaps you should be using a
    ...
    public boolean canScrollVertically(int direction) {
        ...
        if (direction > 0) {
            ...
            return lastPosition < mItemCount || lastBottom > getHeight() - getPaddingBottom();
        } else {
            final int firstTop = getChildAt(0).getTop();
            return firstPosition > 0 || firstTop < getPaddingTop();
        }
    }
    ...
    /**
     * Important: this method will leave offscreen views attached if they are required to maintain the invariant that
     * child view with index i is always the view corresponding to position mFirstPosition + i.
     */
    ...
    /**
     * @return column that the next view filling upwards should occupy. This is the bottom-most position available for a
     *         single-column item.
     */
    ...
    /**
     * Obtain a populated view from the adapter. If optScrap is non-null and is not reused it will be placed in the
     * recycle bin.
     * 
     * @param position
    ...
    public interface SelectionBoundsAdjuster {
         ...
         * 
         * @param bounds
         *            On call, this contains the bounds the list has selected for the item (that is the bounds of the
    ...
    public static class AdapterContextMenuInfo implements ContextMenu.ContextMenuInfo {
        ...
        /**
         * The child view for which the context menu is being displayed. This will be one of the children of this
         * AdapterView.
         */
    ...
    /**
     * @return The callback to be invoked with an item in this AdapterView has been clicked and held, or null id no
